...
class TestParser(unittest.TestCase):
    def test_process_log_line_call(self):
        ...
        processed_json_str = processed_line[
            processed_line.find("[") + 1 : processed_line.rfind("]")
        ]
        expected_json_str = expected_output[
            expected_output.find("[") + 1 : expected_output.rfind("]")
        ]
    ...
    def test_process_log_line_call_result(self):
        ...
        log_line = (
            'CP456 : send [3, "67890", {"status": "Accepted", "transactionId": 987}]'
        )
    ...
    def test_process_log_line_invalid_json(self):
        ...
        log_line = (
            'CP789 : receive [2, "abc", "Action", {key: "value"}]'  # Invalid JSON
        )
        cp_id_token = hash_value("CP789")
        expected_output = (
            f'{cp_id_token} : receive message [2, "abc", "Action", {{key: "value"}}]'
        )
