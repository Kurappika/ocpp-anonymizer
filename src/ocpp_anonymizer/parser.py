...
LOG_LINE_REGEX = re.compile(
    r"^(.*?)\s*:\s*(receive|send)\s+(?:message\s+)?\[(.*?)]\s*$", re.IGNORECASE
)
...
def process_log_line(log_line: str) -> str:
    ...
    if message_type_id == 2:  # CALL request/indication
        ...
        if len(ocpp_message) == 3:
            ...
            anonymized_payload = anonymize_payload(
                "CONF_MESSAGES_NESTED_CHECK", payload
            )
    ...
    anonymized_json_content = json.dumps(ocpp_message)[
        1:-1
    ]  # Dump and strip surrounding brackets
